...
# 説明変数に「間取り」を追加
# Add "間取り" (floor plan) to the explanatory variables
df_model = df[["log_家賃_per_㎡", "築年数_num", "徒歩分_num", "面積_num", "駅名", "間取り"]].dropna()

# 説明変数のダミー化（「間取り」もダミー化の対象に含める）
# Dummy encoding of explanatory variables (including "間取り")
X = pd.get_dummies(df_model[["築年数_num", "徒歩分_num", "面積_num", "駅名", "間取り"]], drop_first=True)
y = df_model["log_家賃_per_㎡"]

# スケーリング（Lassoは標準化が必要）
# Scaling (Standardization is required for Lasso)
scaler = StandardScaler()
X_scaled = scaler.fit_transform(X)

# ----------------------------
# Lasso回帰（交差検証でα選択）
# Lasso Regression (Alpha selection by cross-validation)
# ----------------------------
...
# ----------------------------
# 予測値を元のデータに追加し保存
# Add prediction values to the original data and save
# ----------------------------
